...
//            Log.d("tester","cara: " + vertexIndices[1]);
            ...
            indList.add((short)(vertex1-1));

            /** indice 2*/
//            Log.d("tester","cara 2: " + vertexIndices[2]);
            String indices2[] = vertexIndices[2].split("/");
            short vertex2 = Short.parseShort(indices2[0]);
            indList.add((short)(vertex2-1));

            /** indice 3*/
//            Log.d("tester","cara 3: " + vertexIndices[3]);
            String indices3[] = vertexIndices[3].split("/");
            short vertex3 = Short.parseShort(indices3[0]);
            indList.add((short)(vertex3-1));

        }

        indices = new short[indList.size()];

//        Log.d("tictoc","coords length "+ coords.length);

        for (int i =0; i<indList.size() ; i++){
//            Log.d("tictocc","i: "+ indList.get(i));
            indices[i] = indList.get(i);
        }

        ByteBuffer ibb = ByteBuffer.allocateDirect(indices.length * vertexStride);
        ibb.order(ByteOrder.nativeOrder());
        facesBuffer = ibb.asShortBuffer();
